				...
				public void actionPerformed(java.awt.event.ActionEvent e) {
					...
					if (bloaterFlag || deepNestingFlag || excessiveCommentingFlag
						...
						if (test.isFile() || test.isDirectory()) {
							...
							detector.clearSmells();

						}else{JOptionPane.showMessageDialog(null,
								"The folder or file you have chosen is not correct. \nIt is either empty" +
								" or invalid. Please re-select.",
								"Computer Says No.",
								JOptionPane.WARNING_MESSAGE);}
					}else{JOptionPane.showMessageDialog(null,
							"You have not selected a smell to detect. Please check one at\n least one smell to detect.",
							"Computer Says No.",
							JOptionPane.WARNING_MESSAGE);}
				}
			});
